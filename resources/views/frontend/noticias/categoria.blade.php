...
<div class="container">
    <section id="categories">
        @foreach($categorias as $c)
            @if($loop->last)
            <span><a href="{{ route('categoria_individual', ['id' => $c->id ]) }}" style="color:#{{ $c->hexcode }};@if($categoria == $c->id)font-size:5vmin;@endif">{{ $c->nombre }}</a></span>
            @else
            <span><a href="{{ route('categoria_individual', ['id' => $c->id ]) }}" style="color:#{{ $c->hexcode }};@if($categoria == $c->id)font-size:5vmin;@endif">{{ $c->nombre }}</a><b>|</b></span>
            @endif
        @endforeach
    </section>
    <div id="infinite"> 
...
@section('javascripts')
    ...
    <script src="{{ asset('js/noticias/categoria.js') }}"></script>
@endsection